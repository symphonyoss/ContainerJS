...
ipc.on(IPC_SSF_FLASH_FRAME, (e, id, nonce, args) => {
  const messageInfo = [e.sender, IPC_SSF_FLASH_FRAME, nonce];
  const [flag] = args;
...
ipc.on(IPC_SSF_GET_WINDOW_BOUNDS, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_GET_WINDOW_BOUNDS, nonce];
...
ipc.on(IPC_SSF_GET_WINDOW_MAXIMUM_SIZE, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_GET_WINDOW_MAXIMUM_SIZE, nonce];
  callWindowFunction(id, messageInfo, win => win.getMaximumSize());
});

ipc.on(IPC_SSF_GET_WINDOW_MINIMUM_SIZE, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_GET_WINDOW_MINIMUM_SIZE, nonce];
  callWindowFunction(id, messageInfo, win => win.getMinimumSize());
});

ipc.on(IPC_SSF_GET_WINDOW_PARENT, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_GET_WINDOW_PARENT, nonce];
  callWindowFunction(id, messageInfo, win => win.getParentWindow());
});

ipc.on(IPC_SSF_GET_WINDOW_POSITION, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_GET_WINDOW_POSITION, nonce];
  callWindowFunction(id, messageInfo, win => win.getPosition());
});

ipc.on(IPC_SSF_GET_WINDOW_SIZE, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_GET_WINDOW_SIZE, nonce];
  callWindowFunction(id, messageInfo, win => win.getSize());
});

ipc.on(IPC_SSF_GET_WINDOW_TITLE, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_GET_WINDOW_TITLE, nonce];
  callWindowFunction(id, messageInfo, win => win.getTitle());
});

ipc.on(IPC_SSF_WINDOW_HAS_SHADOW, (e, id, nonce, args) => {
  const messageInfo = [e.sender, IPC_SSF_WINDOW_HAS_SHADOW, nonce];
...
ipc.on(IPC_SSF_IS_WINDOW_ALWAYS_ON_TOP, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_IS_WINDOW_ALWAYS_ON_TOP, nonce];
  callWindowFunction(id, messageInfo, win => win.isAlwaysOnTop());
});

ipc.on(IPC_SSF_IS_WINDOW_MAXIMIZABLE, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_IS_WINDOW_MAXIMIZABLE, nonce];
  callWindowFunction(id, messageInfo, win => win.isMaximizable());
});

ipc.on(IPC_SSF_IS_WINDOW_MAXIMIZED, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_IS_WINDOW_MAXIMIZED, nonce];
  callWindowFunction(id, messageInfo, win => win.isMaximized());
});

ipc.on(IPC_SSF_IS_WINDOW_MINIMIZABLE, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_IS_WINDOW_MINIMIZABLE, nonce];
  callWindowFunction(id, messageInfo, win => win.isMinimizable());
});

ipc.on(IPC_SSF_IS_WINDOW_MINIMIZED, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_IS_WINDOW_MINIMIZED, nonce];
  callWindowFunction(id, messageInfo, win => win.isMinimized());
});

ipc.on(IPC_SSF_IS_WINDOW_RESIZABLE, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_IS_WINDOW_RESIZABLE, nonce];
  callWindowFunction(id, messageInfo, win => win.isResizable());
});

ipc.on(IPC_SSF_LOAD_URL, (e, id, nonce, args) => {
  const messageInfo = [e.sender, IPC_SSF_LOAD_URL, nonce];
  const [url] = args;
  callWindowFunction(id, messageInfo, win => win.loadURL(url));
});

ipc.on(IPC_SSF_RELOAD_WINDOW, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_RELOAD_WINDOW, nonce];
  callWindowFunction(id, messageInfo, win => win.reload());
});

ipc.on(IPC_SSF_RESTORE_WINDOW, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_RESTORE_WINDOW, nonce];
  callWindowFunction(id, messageInfo, win => win.restore());
});

ipc.on(IPC_SSF_SET_WINDOW_ALWAYS_ON_TOP, (e, id, nonce, args) => {
  const messageInfo = [e.sender, IPC_SSF_SET_WINDOW_ALWAYS_ON_TOP, nonce];
  const [alwaysOnTop] = args;
  callWindowFunction(id, messageInfo, win => win.setAlwaysOnTop(alwaysOnTop));
});

ipc.on(IPC_SSF_SET_WINDOW_BOUNDS, (e, id, nonce, args) => {
  const messageInfo = [e.sender, IPC_SSF_SET_WINDOW_BOUNDS, nonce];
  const [bounds] = args;
  callWindowFunction(id, messageInfo, win => win.setBounds(bounds));
});

ipc.on(IPC_SSF_SET_WINDOW_ICON, (e, id, nonce, args) => {
  const messageInfo = [e.sender, IPC_SSF_SET_WINDOW_BOUNDS, nonce];
  const [icon] = args;
  callWindowFunction(id, messageInfo, win => win.setIcon(icon));
});

ipc.on(IPC_SSF_SET_WINDOW_MAXIMIZABLE, (e, id, nonce, args) => {
  const messageInfo = [e.sender, IPC_SSF_SET_WINDOW_MAXIMIZABLE, nonce];
  const [maximizable] = args;
  callWindowFunction(id, messageInfo, win => win.setMaximizable(maximizable));
});

ipc.on(IPC_SSF_SET_WINDOW_MAXIMUM_SIZE, (e, id, nonce, args) => {
  const messageInfo = [e.sender, IPC_SSF_SET_WINDOW_MAXIMUM_SIZE, nonce];
  const [width, height] = args;
  callWindowFunction(id, messageInfo, win => win.setMaximumSize(width, height));
});

ipc.on(IPC_SSF_SET_WINDOW_MINIMIZABLE, (e, id, nonce, args) => {
  const messageInfo = [e.sender, IPC_SSF_SET_WINDOW_MINIMIZABLE, nonce];
  const [minimizable] = args;
  callWindowFunction(id, messageInfo, win => win.setMinimizable(minimizable));
});

ipc.on(IPC_SSF_SET_WINDOW_MINIMUM_SIZE, (e, id, nonce, args) => {
  const messageInfo = [e.sender, IPC_SSF_SET_WINDOW_MINIMUM_SIZE, nonce];
  const [width, height] = args;
  callWindowFunction(id, messageInfo, win => win.setMinimumSize(width, height));
});

ipc.on(IPC_SSF_SET_WINDOW_POSITION, (e, id, nonce, args) => {
  const messageInfo = [e.sender, IPC_SSF_SET_WINDOW_POSITION, nonce];
  const [x, y] = args;
  callWindowFunction(id, messageInfo, win => win.setPosition(x, y));
});

ipc.on(IPC_SSF_SET_WINDOW_RESIZABLE, (e, id, nonce, args) => {
  const messageInfo = [e.sender, IPC_SSF_SET_WINDOW_RESIZABLE, nonce];
  const [resizable] = args;
  callWindowFunction(id, messageInfo, win => win.setResizable(resizable));
});

ipc.on(IPC_SSF_SET_WINDOW_SIZE, (e, id, nonce, args) => {
  const messageInfo = [e.sender, IPC_SSF_SET_WINDOW_SIZE, nonce];
  const [width, height] = args;
  callWindowFunction(id, messageInfo, win => win.setSize(width, height));
});

ipc.on(IPC_SSF_SET_WINDOW_SKIP_TASKBAR, (e, id, nonce, args) => {
  const messageInfo = [e.sender, IPC_SSF_SET_WINDOW_SKIP_TASKBAR, nonce];
...
ipc.on(IPC_SSF_MAXIMIZE_WINDOW, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_MAXIMIZE_WINDOW, nonce];
  callWindowFunction(id, messageInfo, win => win.maximize());
});

ipc.on(IPC_SSF_MINIMIZE_WINDOW, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_MINIMIZE_WINDOW, nonce];
  callWindowFunction(id, messageInfo, win => win.minimize());
});

ipc.on(IPC_SSF_UNMAXIMIZE_WINDOW, (e, id, nonce) => {
  const messageInfo = [e.sender, IPC_SSF_UNMAXIMIZE_WINDOW, nonce];
